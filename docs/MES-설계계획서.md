...
Modus/
├─ backend/
│  ├─ Modus.Api/            # ASP.NET Core 진입점, 컨트롤러, 인증, 미들웨어
│  ├─ Modus.Domain/         # 엔티티(단일 진실 소스), 열거형
│  ├─ Modus.Infrastructure/ # DbContext(Tenant/Catalog), EF 설정, 마이그레이션, 리포지토리
│  ├─ Modus.Application/    # 서비스/유스케이스 — 생산·품질 로직
│  └─ Modus.Tools/          # 마이그레이터/테넌트 프로비저닝 CLI
├─ web/
